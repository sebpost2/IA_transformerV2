#!/usr/bin/env python
"""
main.py — Inference with the most recent checkpoint
---------------------------------------------------
"""
...
p.add_argument("--img_size", type=int, default=0,
               help="Override image size; 0 = infer from checkpoint")
args = p.parse_args()
...
net = MiniViT(img_size=img_size, patch=meta['patch'], embed=meta['embed'],
              heads=6, ff=meta['embed']*2, layers=8,
...
for p, pr in zip(paths, probs):
    pred = "ARMA" if pr[1] >= 0.5 else "NO ARMA"
