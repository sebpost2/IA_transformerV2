...
class MiniViT:
    ...
    def backward(self, cache, imgs, labels, *, pos_weight: float = 1.0):
        """Backprop con peso para la clase positiva."""
        # limpia gradientes
        ...
        class_w = cp.broadcast_to(class_w, labels.shape)
    ...
    def step(self, lr: float = 3e-4, clip: float = 5.0, wd: float = 0.0,
             momentum: float = 0.9):
        """Actualiza los pesos con SGD y momento."""
        norm = 0.0
        ...
        for p, g, name in self._param_items():
            # Gradiente escalado y con weight decay
    ...
    def save(self, path: str | Path):
        """Guarda los parámetros en formato arr_0, arr_1, … ."""
